...
class ZohoAPI {
    ...
    async updateProduct(productId, auditData) {
        ...
        try {
            ...
            const calculateWeightCategory = (w) => {
                const brackets = [5, 10, 20, 50, 100, 500];
                for (const b of brackets) if (w <= b) return `${b}kg`;
                return '500kg+';
            };
            const weightCategory = calculateWeightCategory(weightKG);
            ...
            if (module === 'Parent_MTP_SKU') {
                ...
                if (boxes.length > 0) {
                    updateData.MTP_Box_Dimensions = boxes.map((b, idx) => ({
                        Box: String(idx + 1),
                        Length: Number(b.length || b.Length),
